    ...
    : ICSharpDependencyGraphObject
{
        ...
        string? consumerNamespace)
    {
        ...
        // The interface and implementation are emitted in the consumer's
        // containing namespace (or `Microsoft.JSInterop` when the consumer
        // omits one). Add a `using` for that namespace so the DI extension
        // class — which always lives in
        // `Microsoft.Extensions.DependencyInjection` so consumers can chain
        // `services.Add...()` calls without an extra `using` — can resolve
        // both type names. Without this, any consumer placing their
        // interface outside `Microsoft.JSInterop` produced a DI extension
        // that referenced undefined types.
        var effectiveNamespace = string.IsNullOrWhiteSpace(consumerNamespace)
            ? "Microsoft.JSInterop"
            : consumerNamespace!;

        var usings = string.Equals(effectiveNamespace, "Microsoft.JSInterop", StringComparison.Ordinal)
