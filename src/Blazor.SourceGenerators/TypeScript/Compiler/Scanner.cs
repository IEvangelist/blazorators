...
internal sealed class Scanner
{
    ...
    internal Scanner(ScriptTarget languageVersion, bool skipTrivia, LanguageVariant languageVariant, string text, int start = 0, int? length = null)
    {
    ...
    internal static bool LookupInUnicodeMap(CharacterCode @char, int[] map)
    {
        var code = (int)@char;
        if (code < map[0])
        {
    ...
    internal static TypeScriptSyntaxKind StringToToken(string s) => s_textToTokenMap[s];

    internal static List<int> ComputeLineStarts(string text)
    {
        List<int> result = [];
        var pos = 0;
        var lineStart = 0;
...
linefeed: result.Add(lineStart);
    ...
    internal static int GetPositionOfLineAndCharacter(SourceFile sourceFile, int line, int character) =>
        ComputePositionOfLineAndCharacter(GetLineStarts(sourceFile), line, character);

    internal static int ComputePositionOfLineAndCharacter(int[] lineStarts, int line, int character) =>
        lineStarts[line] + character;

    internal static int[] GetLineStarts(ISourceFileLike sourceFile) =>
        sourceFile.LineMap = [.. ComputeLineStarts(sourceFile.Text)];

    internal static LineAndCharacter ComputeLineAndCharacterOfPosition(int[] lineStarts, int position)
    {
    ...
    internal static LineAndCharacter GetLineAndCharacterOfPosition(
    ...
    internal static bool IsDigit(CharacterCode @char) =>
...
linefeed: pos++;
    ...
    internal static bool IsShebangTrivia(string text, int _) =>
                    ...
                    if (nextChar is CharacterCode.Slash or CharacterCode.Asterisk)
                    {
                        ...
                        if (collecting)
                        {
                            if (hasPendingCommentRange)
                            {
                                ...
                                if (!reduce && !Equals(accumulator, default(U)))
                                {
...
breakScan:
    ...
    internal static U ForEachLeadingCommentRange<T, U>(
        string text,
        int pos,
        Func<(int pos, int end, TypeScriptSyntaxKind kind, bool hasTrailingNewLine, T state, U memo), U> callback,
        T state) =>
    ...
    internal static U ForEachTrailingCommentRange<T, U>(
        string text,
        int pos,
        Func<(int pos, int end, TypeScriptSyntaxKind kind, bool hasTrailingNewLine, T state, U memo), U> callback,
        T state) =>
    ...
    internal static string GetShebang(string text) => s_shebangTriviaRegex.Test(text)
    ...
    internal int ScanHexDigits(int minCount, bool scanAsManyAsPossible)
    {
        ...
        while (digits < minCount || scanAsManyAsPossible)
        {
            ...
            else if (ch is >= CharacterCode.A and <= CharacterCode.F)
            {
    ...
    internal string ScanString(bool allowEscapes = true)
    {
        ...
        while (true)
        {
            if (TextPos >= _end)
            {
                result.Append(_text.SubString(start, TextPos));
                IsUnterminated = true;
            ...
            {
                result.Append(_text.SubString(start, TextPos));
                result.Append(ScanEscapeSequence());
                start = TextPos;
                continue;
            }
            if (IsLineBreak(ch))
    ...
    internal TypeScriptSyntaxKind ScanTemplateAndSetTokenValue()
    {
        ...
        while (true)
        {
            if (TextPos >= _end)
            {
                contents.Append(_text.SubString(start, TextPos));
                IsUnterminated = true;
            ...
            {
                contents.Append(_text.SubString(start, TextPos));
                contents.Append(ScanEscapeSequence());
                start = TextPos;
                continue;
            }
...
linefeed: return "";
    ...
    internal static string Utf16EncodeAsString(int codePoint)
    {
    ...
    internal string ScanIdentifierParts()
    {
        ...
        while (TextPos < _end)
        {
            ...
            else if (ch is CharacterCode.Backslash)
            {
                ...
                result.Append(_text.SubString(start, TextPos));
                result.Append(StringExtensions.FromCharCode((int)ch));
    ...
    internal TypeScriptSyntaxKind GetIdentifierToken()
    {
        ...
        if (len is >= 2 and <= 11)
        {
            ...
            if (ch is >= CharacterCode.a and <= CharacterCode.z && s_textToTokenMap.TryGetValue(TokenValue, out var value))
            {
                Token = value;
                return Token;
            }
...
#pragma warning restore S1121 // Assignments should not be made from within sub-expressions
    ...
    internal TypeScriptSyntaxKind ReScanSlashToken()
    {
        if (Token is TypeScriptSyntaxKind.SlashToken or TypeScriptSyntaxKind.SlashEqualsToken)
        {
            ...
            while (true)
            {
                ...
                else if (ch is CharacterCode.Slash && !inCharacterClass)
                {
                ...
                else if (ch is CharacterCode.OpenBracket)
                {
                ...
                else if (ch is CharacterCode.CloseBracket)
                {
    ...
    internal T SpeculationHelper<T>(Func<T> callback, bool isLookahead)
    {
        ...
        var result = callback();
        if (result is null || (result is bool && !Convert.ToBoolean(result)) || isLookahead)
        {
            TextPos = savePos;
