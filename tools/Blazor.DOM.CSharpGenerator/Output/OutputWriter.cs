...
public static class OutputVerifier
{
    ...
    {
        var r1 = run1.ToDictionary(f => f.RelativePath, StringComparer.OrdinalIgnoreCase);
        var r2 = run2.ToDictionary(f => f.RelativePath, StringComparer.OrdinalIgnoreCase);

        var mismatches = new List<FileMismatch>();
        var inR1NotR2 = r1.Keys.Except(r2.Keys, StringComparer.OrdinalIgnoreCase).ToList();
        var inR2NotR1 = r2.Keys.Except(r1.Keys, StringComparer.OrdinalIgnoreCase).ToList();

        foreach (var path in r1.Keys.Intersect(r2.Keys, StringComparer.OrdinalIgnoreCase))
        {
            var f1 = r1[path];
            var f2 = r2[path];
    ...
    {
        var files = Directory.GetFiles(directory, "*.g.cs", SearchOption.AllDirectories);
