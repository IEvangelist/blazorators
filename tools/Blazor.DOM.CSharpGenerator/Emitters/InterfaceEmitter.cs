...
public sealed class InterfaceEmitter(TypeResolver typeResolver, string generatorVersion, string ns)
{
    ...
    private MethodBuildResult BuildMethod(MemberModel method, string symbolName, int declOrdinal)
    {
        ...
        var paramList = method.Parameters.OrderBy(p => p.Ordinal).ToList();
        ...
        foreach (var decl in allDecls)
        {
            foreach (var heritage in decl.Heritage)
            {
                ...
                foreach (var typeNode in heritage.Types)
                {
                    ...
                    if (!typeResolver.IsKnownSymbol(hrt.Expression))
                    {
                        throw new TypeProjectionException(
                            $"Interface '{symbolName}' decl[{decl.Ordinal}] extends unknown symbol '{hrt.Expression}'. " +
                            "Add it to the symbol index or provide an emitter override.",
                            $"{symbolName}/extends/{hrt.Expression}");
                    }

                    if (!typeResolver.IsInterfaceOrMixin(hrt.Expression))
                    {
                        throw new TypeProjectionException(
                            $"Interface '{symbolName}' decl[{decl.Ordinal}] extends '{hrt.Expression}' which has classification " +
                            $"'{typeResolver.GetClassification(hrt.Expression)}', not interface/mixin. " +
                            "Only interface/mixin bases are valid in a C# interface base clause.",
                            $"{symbolName}/extends/{hrt.Expression}");
                    }

                    var csBase = $"I{Naming.ToCSharpTypeName(hrt.Expression)}";
